...
// Custom renderer to ensure links open correctly and are not sanitized
const renderer = new Renderer();
renderer.link = ({ href, title, tokens }) => {
  const text = tokens.map((t: any) => t.raw || t.text || '').join('');
  const titleAttr = title ? ` title="${title}"` : '';
  const isExternal = href && (href.startsWith('http') && !href.includes('signallaboratories.co.uk'));
  const targetAttr = isExternal ? ' target="_blank" rel="noopener noreferrer"' : '';
  return `<a href="${href}"${titleAttr}${targetAttr} class="text-blue-600 underline font-medium">${text}</a>`;
};
      ...
      <article className="rounded-2xl border border-black/10 bg-white p-6 md:p-8">
        ...
        {heroUrl ? (
          ...
          dangerouslySetInnerHTML={{
            __html: marked(post.contentMarkdown ?? "", { breaks: true, renderer }) as string,
          }}
        />
